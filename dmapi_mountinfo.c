...
//printk(KERN_NOTICE "dm_fsys_map_by_fstype: entry\n");

	list_for_each(p, &dm_fsys_map) {
		m = list_entry(p, dm_vector_map_t, ftype_list);
		if (m->f_type == fstype) {
			proto = m;
//printk(KERN_NOTICE "dm_fsys_map_by_fstype: found entry\n");
			break;
		}
	}
if(!proto) printk(KERN_NOTICE "dm_fsys_map_by_fstype: did not find entry\n");
	return proto;
}
...
//printk("dm_fsys_map_by_sb: Called with type %s\n", sb->s_type->name);
	proto = dm_fsys_map_by_fstype(sb->s_type);
	if(proto == NULL) {
//printk("dm_fsys_map_by_sb: return NULL\n");
		return NULL;
	}
	...
	list_for_each(p, &proto->sb_list) {
		...
		if (m->sb == sb) {
			foundmap = m;
//printk(KERN_NOTICE "dm_fsys_map_by_sb: found map entry\n");
			break;
		}
	}
if(!foundmap) printk(KERN_NOTICE "dm_fsys_map_by_sb: did not find map entry\n");
	...
	struct super_block	*sb)
{
	...
	if (map == NULL) {
		printk("dm_fsys_ops: NULL from dm_fsys_map_by_sb\n");
		proto = dm_fsys_map_by_fstype(sb->s_type);
		if( proto == NULL ) printk("dm_fsys_ops: NULL from dm_fsys_map_by_fstype\n");
	}

	spin_unlock(&dm_fsys_lock);

	if ((map == NULL) && (proto == NULL)) {
		printk(KERN_NOTICE "dm_fsys_ops: returning NULL from map and proto\n");
		return NULL;
	}
	...
	if (map == NULL) {
		...
		printk(KERN_NOTICE "dm_fsys_ops: map is null\n");
		ASSERT(proto);
		...
		if (m->vptr == NULL) {
			...
			printk(KERN_NOTICE "dm_fsys_ops: dm_query_fsys_for_vector is NULL\n");
	...
	if(!map) printk(KERN_NOTICE "dm_fsys_ops: returning NULL map\n");
	if(!map->dmapiops) printk(KERN_NOTICE "dm_fsys_ops: returning NULL dmapiops\n");
	printk(KERN_NOTICE "dm_fsys_ops: returning valid\n");
	return map->dmapiops;
}
	...
	struct filesystem_dmapi_operations *dmapiops)
{
	...
	ftype_list();
	spin_unlock(&dm_fsys_lock);
	printk(KERN_NOTICE "dmapi_register successful return\n");
}
