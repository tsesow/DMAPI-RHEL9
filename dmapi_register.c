	...
	unsigned long	*lcp)		/* address of returned lock cookie */
{
	...
	printk(KERN_NOTICE "dm_find_fsreg_and_lock: entry\n");
	for (;;) {
		*lcp = mutex_spinlock(&dm_reg_lock);

		if ((fsrp = dm_find_fsreg(fsidp)) == NULL) {
			printk(KERN_ERR "dm_find_fsreg_and_lock: NULL return from dm_find_fsreg\n");
			mutex_spinunlock(&dm_reg_lock, *lcp);
			return(NULL);
		}
		if (spin_trylock(&fsrp->fr_lock)) {
			printk(KERN_ERR "dm_find_fsreg_and_lock: spin_trylock success\n");
	...
	printk(KERN_ERR "dm_find_fsreg_and_lock: impossible return\n");
}
...
printk(KERN_NOTICE "dm_path_to_hdl: Call pathlookup with %s\n", name);

	error = path_lookup(name, LOOKUP_POSITIVE, &nd);
	kfree(name);
	if (error){
printk(KERN_ERR "dm_path_to_hdl: Path_lookup failure (%d)\n", error);
		return error;
	}
printk(KERN_NOTICE "dm_path_to_hdl: Path_lookup succeeded\n");
...
printk(KERN_ERR "dm_path_to_hdl: No longer in dmapi-capable filesystem failure\n");
		iput(inode);
		return -EINVAL;
	}

	/* we need the inode */
printk(KERN_NOTICE "dm_path_to_hdl: get inode \n");
	error = dm_ip_to_handle(inode, &handle);
	iput(inode);
	if (error){
printk(KERN_ERR "dm_path_to_hdl: dm_ip_to_handle failed\n");
		return(error);
	}
...
printk(KERN_NOTICE "dm_path_to_fshdl entry, path %s\n", path);

	/* XXX get things straightened out so getname() works here? */
	if(!(len = strnlen_user(path, PATH_MAX))) {
printk(KERN_ERR "dm_path_to_fshdl:  path null\n");
		return(-EFAULT);
	}

	if (len == 1){
printk(KERN_ERR "dm_path_to_fshdl:  path too short\n");
		return(-ENOENT);
	}

	if (len > PATH_MAX){
printk(KERN_ERR "dm_path_to_fshdl:  path too long\n");
		return(-ENAMETOOLONG);
	}
	...
	if (copy_from_user(name, path, len)) {
printk(KERN_ERR "dm_path_to_fshdl:  copy from user failed \n");
		kfree(name);
		return(-EFAULT);
	}

printk(KERN_NOTICE "dm_path_to_fshdl:  about to call path_lookup for %s\n", name);
	error = path_lookup(name, LOOKUP_POSITIVE|LOOKUP_FOLLOW, &nd);
	kfree(name);
	if (error){
printk(KERN_ERR "dm_path_to_fshdl:  path_lookup failed \n");
		return error;
	}
...
//	path_release(&nd);  TSS
	...
	dops = dm_fsys_ops(inode->i_sb);
	if (dops == NULL) {
printk(KERN_ERR "dm_path_to_fshdl:  No longer in a dmapi-capable filesystem...Toto\n");
		/* No longer in a dmapi-capable filesystem...Toto */
		iput(inode);
		return -EINVAL;
	}
	...
	if (error) {
printk(KERN_ERR "dm_path_to_fshdl:  dm_ip_to_handle failed\n");
		return(error);
	}

	if ((fsrp = dm_find_fsreg_and_lock(&handle.ha_fsid, &lc)) == NULL){
printk(KERN_ERR "dm_path_to_fshdl:  dm_find_fsreg_and_lock failed \n");
		return(-EBADF);
	}
	mutex_spinunlock(&fsrp->fr_lock, lc);
	...
	hlen = DM_FSHSIZE;
	if(copy_to_user(hanp, &handle, (int)hlen)){
printk(KERN_NOTICE "dm_path_to_fshdl:  copy_to_user failed \n");
		return(-EFAULT);
	}
	if(put_user(hlen,hlenp))
		return(-EFAULT);
printk(KERN_NOTICE "dm_path_to_fshdl:  return success 0\n");
	return 0;
}
...
// found:
//	dget_locked(dentry);
//	dentry->d_flags |= DCACHE_REFERENCED; /*TSS*/
//	spin_unlock(&dcache_lock);
//	iput(inode);
//	return dentry;
//}
	...
	int		*rvp)
{
	...
	printk(KERN_NOTICE "dm_open_by_handle_rvp: Entry\n");
...
//	dentry = d_alloc_anon(inodep);
	...
	if (dentry == NULL) {
	//	iput(inodep); /* Might not need this with d_obtain_alias  TSS */
	...
	/* Still trying to figure out the correct dcache handling */
	iput(inodep); /* Have to do this for newer kernels TSS*/
	dput(dentry);
